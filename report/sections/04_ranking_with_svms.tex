...
Ý tưởng chính của SVM-ranking là biến bài toán ranking theo từng cặp thành một bài toán phân loại nhị phân trên các vector hiệu. Thay vì học trực tiếp từ từng điểm \(x\), thuật toán học từ các cặp \((x_i,x'_i)\) bằng cách xét hiệu đặc trưng giữa hai đối tượng. Nhờ phép biến đổi này, các công cụ quen thuộc của SVM như margin, slack variable, regularization và kernel trick có thể được áp dụng trực tiếp cho bài toán ranking.

\begin{figure}[H]
    \centering
    \begin{tikzpicture}[>=Stealth, node distance=1.6cm]
        \node[draw, rounded corners, fill=gray!10, align=center, minimum width=3.5cm, minimum height=1.05cm] (pair)
        {Cặp đối tượng\\ \((x_i,x'_i,y_i)\)};
        \node[draw, rounded corners, fill=gray!10, align=center, minimum width=3.8cm, minimum height=1.05cm, right=of pair] (diff)
        {Vector hiệu\\ \(\Psi(x_i,x'_i)=\Phi(x'_i)-\Phi(x_i)\)};
        \node[draw, rounded corners, fill=gray!10, align=center, minimum width=3.5cm, minimum height=1.05cm, right=of diff] (svm)
        {SVM nhị phân\\ trên \((z_i,y_i)\)};
        \draw[->, thick] (pair) -- (diff);
        \draw[->, thick] (diff) -- (svm);
    \end{tikzpicture}
    \caption{Ý tưởng chính của SVM-ranking: biến mỗi cặp ưu tiên thành một mẫu phân loại trên vector hiệu.}
    \label{fig:section4-svm-ranking-pipeline}
\end{figure}
...
\end{equation}
Biến \(\xi_i\) đo mức độ vi phạm margin của cặp thứ \(i\). Nếu \(\xi_i=0\), cặp đó đạt margin ít nhất \(1\); nếu \(0<\xi_i<1\), cặp vẫn có thể đúng chiều nhưng không đạt đủ margin; nếu \(\xi_i\geq 1\), cặp có thể bị xếp sai hoặc nằm quá gần biên quyết định.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[>=Stealth, node distance=1.1cm]
        \node[draw, rounded corners, fill=gray!10, align=center, minimum width=4.1cm, minimum height=1.1cm] (norm)
        {\(\frac{1}{2}\|w\|^2\)\\ margin lớn, mô hình đơn giản};
        \node[draw, rounded corners, fill=gray!10, align=center, minimum width=4.1cm, minimum height=1.1cm, right=of norm] (slack)
        {\(C\sum_i \xi_i\)\\ phạt vi phạm margin};
        \node[draw, rounded corners, fill=gray!20, align=center, minimum width=4.5cm, minimum height=1.1cm, below=1.2cm of $(norm)!0.5!(slack)$] (obj)
        {Mục tiêu SVM-ranking\\ cân bằng đơn giản hoá và khớp dữ liệu};
        \draw[->, thick] (norm.south) -- (obj.north west);
        \draw[->, thick] (slack.south) -- (obj.north east);
    \end{tikzpicture}
    \caption{Hai thành phần trong hàm mục tiêu của SVM-ranking: chuẩn của \(w\) kiểm soát độ phức tạp, còn các biến slack đo mức vi phạm margin.}
    \label{fig:section4-primal-objective-components}
\end{figure}
...
\end{equation}
Công thức~\eqref{eq:section4-regularized-pairwise-hinge} cho thấy SVM-ranking không tối ưu trực tiếp lỗi pairwise misranking dạng chỉ thị, mà tối ưu một hàm mất mát lồi thay thế là pairwise hinge loss. Hàm mất mát này phạt các cặp bị xếp sai và cả các cặp đúng nhưng chưa đạt margin \(1\).
...
\begin{table}[H]
    ...
    \begin{tabular}{lll}
        \toprule
        \textbf{Thành phần} & \textbf{SVM phân loại} & \textbf{SVM-ranking} \\
        \midrule
        Mẫu huấn luyện & \((x_i,y_i)\) & \((x_i,x'_i,y_i)\) \\
        Đặc trưng đầu vào & \(x_i\) & \(\Psi(x_i,x'_i)=\Phi(x'_i)-\Phi(x_i)\) \\
        Điều kiện margin & \(y_i w^\top x_i\geq 1-\xi_i\) & \(y_i w^\top\Psi(x_i,x'_i)\geq 1-\xi_i\) \\
        Mục tiêu & phân loại đúng từng điểm & xếp đúng từng cặp ưu tiên \\
        \bottomrule
    \end{tabular}
    \caption{SVM-ranking có cùng cấu trúc với SVM phân loại, nhưng dữ liệu đầu vào là các vector hiệu sinh từ từng cặp đối tượng.}
    \label{tab:section4-svm-vs-ranking-svm}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{figures/sec4_svm_ranking_geometry.pdf}
    \caption{Hình học của SVM-ranking trong không gian vector hiệu \(z=\Psi(x,x')\). Đường \(w^\top z=0\) đóng vai trò siêu phẳng phân tách, còn hai đường biên margin tương ứng với \(w^\top z=1\) và \(w^\top z=-1\). Các điểm có nhãn \(y=+1\) và \(y=-1\) biểu diễn các cặp ưu tiên sau khi được biến đổi thành vector hiệu.}
    \label{fig:section4-svm-ranking-geometry}
\end{figure}

Hình~\ref{fig:section4-svm-ranking-geometry} minh hoạ trực quan cách SVM-ranking hoạt động. Sau khi chuyển cặp đối tượng thành vector hiệu \(z\), bài toán ranking trở thành bài toán tìm siêu phẳng phân tách hai nhóm vector hiệu. Các điểm nằm đúng phía và cách xa siêu phẳng ít nhất một khoảng margin sẽ không bị phạt; các điểm nằm trong vùng margin hoặc sai phía sẽ tạo ra slack và đóng góp vào pairwise hinge loss.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[>=Stealth, node distance=1.2cm]
        \node[draw, rounded corners, fill=gray!10, align=center, minimum width=3.2cm, minimum height=1cm] (k)
        {Kernel gốc\\ \(K(x,x')\)};
        \node[draw, rounded corners, fill=gray!10, align=center, minimum width=4.2cm, minimum height=1cm, right=of k] (kp)
        {Kernel trên cặp\\ \(K'_{ij}=\Psi_i\cdot\Psi_j\)};
        \node[draw, rounded corners, fill=gray!10, align=center, minimum width=3.5cm, minimum height=1cm, right=of kp] (dual)
        {SVM dual\\ không cần tính \(\Phi\)};
        \draw[->, thick] (k) -- (kp);
        \draw[->, thick] (kp) -- (dual);
    \end{tikzpicture}
    \caption{Kernel trick trong SVM-ranking: từ kernel giữa các đối tượng ban đầu, ta xây dựng kernel giữa các cặp thông qua vector hiệu.}
    \label{fig:section4-pairwise-kernel-trick}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabular}{lll}
        ...
        \midrule
        Tạo toàn bộ cặp & \(O(n^2)\) & Có \(\binom{n}{2}\) cặp \\
        Tạo vector hiệu & \(O(n^2d)\) & Mỗi cặp cần hiệu hai vector \(d\) chiều \\
        Huấn luyện tuyến tính trên cặp & phụ thuộc solver, thường theo \(m\) và \(d\) & \(m=O(n^2)\) nếu dùng mọi cặp \\
        Lưu kernel trên cặp & \(O(m^2)\) & Có thể thành \(O(n^4)\) nếu \(m=O(n^2)\) \\
        \bottomrule
    \end{tabular}
    \caption{Tóm tắt chi phí chính của SVM-ranking khi làm việc trên dữ liệu pairwise.}
    \label{tab:section4-svm-ranking-complexity}
\end{table}
...
Hình~\ref{fig:section4-svm-ranking-geometry} giúp trực quan hoá ý tưởng này: sau khi biến đổi mỗi cặp thành vector hiệu \(z\), SVM-ranking tìm một siêu phẳng \(w^\top z=0\) để phân tách các cặp được gán nhãn \(+1\) và \(-1\). Các đường margin cho biết vùng an toàn mà mô hình mong muốn các vector hiệu nằm ngoài.
