...
Phần này đã giới thiệu các khái niệm nền tảng của bài toán ranking. Ranking có thể được sinh ra từ một hàm điểm hoặc từ quan hệ ưu tiên theo từng cặp. Trong score-based ranking, mô hình học \(h:\mathcal{X}\rightarrow\mathbb{R}\); trong preference-based ranking, mô hình học trực tiếp \(h:\mathcal{X}\times\mathcal{X}\rightarrow[0,1]\). Lỗi ranking thường được đo bằng tỉ lệ cặp bị xếp sai. Riêng trong bipartite ranking, AUC đo tỉ lệ các cặp positive-negative được xếp đúng.
